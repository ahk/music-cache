...
class ID3v2 < DelegateClass(Hash)
  ...
  def decode_tag(name, raw_value)
    ...
    when /^T/
      ...
      if !encoding.nil? && TEXT_ENCODINGS[encoding] && out #&& encoding != @text_encoding_index
        begin
          #out = Iconv.iconv(@options[:encoding], TEXT_ENCODINGS[encoding], out).first
          out = Iconv.iconv("UTF-8//IGNORE", TEXT_ENCODINGS[encoding], out).first
        rescue Iconv::Failure
        end
      end
      # remove padding zeros for textual tags
      out.sub!(/\0*$/, '')
      return out.to_readable_english
    else
      return raw_value
    end
  end
